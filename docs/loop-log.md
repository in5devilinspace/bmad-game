...
## 2026-07-06 — Audio: optional, muted-by-default WebAudio kit (footsteps / alarm sting / confetti chime)

**Item:** "Audio (optional, muted-by-default, single WebAudio oscillator kit): footstep ticks, alarm sting, confetti chime — no external assets."

**Outcome: DONE. Presentation/output only — `<script id="sim">` is BYTE-IDENTICAL to HEAD (sha256 `7e12809b…` before AND after the change).**

**What changed (`src/index.html`):**
- **New `WH.audio` module** — a self-contained IIFE added to the presentation `<script>` block, AFTER the juice module (never inside the sim block). ONE lazily-built `AudioContext`, created on the first user gesture (the existing start click / keydown handlers and the 'm' toggle all call `WH.audio.init()`), per the browser autoplay policy. Everything is SYNTHESIZED — oscillators + one cached 90 ms noise buffer — so there are zero external audio assets and the file still works offline from `file://`.
- **Voices:** footstep ticks = short band-passed noise clicks driven by thief cell-transitions with a global 150 ms throttle (so six thieves never machine-gun); alarm sting = a dissonant ~tritone sawtooth sweep on `ALARM_ARMED` / `ALARM_TRIPPED`; confetti chime = a sine-bell major arpeggio on `LOOT_GRAB` (3 notes) / `GEM_GRAB` (4 notes, brighter); plus a soft low sine thud on `plate`/`trap` and a breathy noise "shhh" whisper cue when a lie is actually planted (`placeLie`).
- **Event sourcing (no duplicated logic):** dramatic beats are driven from the SAME governed chokepoint the juice layer already uses — a single line in `juice.activate(beat)` calls `root.WH.audio.beat(beat.fx)`. Since `activate` only fires when a beat is promoted (one-per-`GAP`=1500 ms cap), the audio inherits that governance for free. Footsteps are sampled from the raid frame (`WH.audio.footsteps(game, dt)`) with their own throttle. Audio does NOT touch the sim RNG (uses `Math.random()` for noise/jitter), so determinism is unaffected.
- **MUTED BY DEFAULT (two independent guarantees):** the master `GainNode` starts at `gain = 0`, AND every voice early-returns while `muted`, so no oscillator/noise node is ever *started* in the muted state. Nothing is audible until the player opts in. Unmute ramps master to a gentle 0.55 ceiling; per-voice envelopes stay well below it (unobtrusive).
- **Discoverable, keyboard-accessible toggle:** an always-on-canvas speaker glyph (top-right, dim + slash when muted / lit + sound-waves when on; zero numerals), hit-tested and clickable in EVERY mode; plus the **'m' key** (intercepted at the top of the keydown handler so it toggles instead of starting a run). Keyboard-accessible per iteration 12 — the canvas is the focusable control (`tabindex=0`, `:focus-visible`); its `aria-label` now advertises "Sound starts off; press M or click the speaker to toggle." Choice persists **in-memory for the session** (a module-level `muted` flag → resets to muted on reload).
- **prefers-reduced-motion:** audio is independent of it by design (that setting governs motion, not sound) but deliberately kept quiet/subtle.
- Read-only `WH._debug.audio()` added (`{ muted, supported, hasCtx, gain }`) so the verifier can inspect the master-gain / context state without driving gameplay.
- Line count **2190 → 2341** (< 2400 cap).

**Verification evidence:**
- `sha256` of the `<script id="sim">` block (lines 21–987) is **identical before and after** (`7e12809b2617d6abf103bd8580941c0c6b0bad5d3d5fb9d29fe759acc34f221f`) → sim untouched.
- `node tests/run-tests.mjs` → **ALL SUITES PASSED** (sim 31/0, gossip 35 assertions, chronicle 21/0, gem-steal 15/0, balance 9/9, outsmart 15/0). No regression (sim byte-unchanged).
- `node --check` via `new vm.Script` on all 3 inline blocks (967 / 1013 / 335 lines): **all SYNTAX OK**.
- **Mock-AudioContext node audit (throwaway, deleted): 17/17.** Extracted the `WH.audio` IIFE, ran it against a counting mock: DEFAULT `muted()===true` with `_actx()===null`; beats/footsteps/cue while muted (pre- AND post-init) start **0 nodes** and never throw; after `init()` master `gain===0`; `toggle()` → `muted()===false` and master `gain===0.55`; `beat('gem')`/`beat('alarm')`/footstep-on-cell-change then start oscillator/buffer nodes without throwing; re-mute → `gain===0`, no new nodes.
- **game-verifier (real headless Chromium 1228, Playwright; audio-node counter + `resume()` hook installed via `addInitScript` BEFORE navigation) → PASS on all 4 gates:** (1) 0 console errors / 0 pageerrors / 0 failed requests, `navStatus 200`, offline; canvas renders the vault; raid reaches playing state with 6 thieves moving (`framesDiffer` on a 1.5 s byte-diff). (2) **Muted by default** — after the start gesture `WH._debug.audio()` = `{muted:true, supported:true, hasCtx:true, gain:0}`; `__audioNodes = 0` after a ~4.5 s muted raid AND in a second probe with thieves *actively moving* (footsteps would fire if unmuted) → still `0`. (3) **Unmute → graph activity, no throw** — press 'm' → `{muted:false, gain:0.550…}`; with genuine sim-driven events `__audioNodes` climbed organically `0→…→15` on real footsteps, then `15→33` on dramatic beats, `thrown:null`, 0 new errors; re-mute 'm' → `{muted:true, gain:0}`; clicking the top-right speaker glyph toggles the same state. (4) speaker glyph visibly drawn top-right (screenshots). Verifier honestly noted `__audioResumed` stayed false because headless Chromium starts the context in `running` state (resume never needed) — does not affect the muted-by-default proof.

**Dirty working tree:** the conversation-start snapshot showed pre-existing dirt in `docs/spec/spec-whisper-heist/*` (modified) + untracked `interfaces.md`/`vault-map.md` + a deleted `.claude/scheduled_tasks.lock` — NONE of it mine; `src/index.html` was clean. This commit touches ONLY this iteration's files: `src/index.html`, `BACKLOG.md`, `docs/loop-log.md`. Throwaway node probes were run from stdin / `/tmp` and left no repo artifacts.
